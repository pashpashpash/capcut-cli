...
```bash
cargo run -- library sound judge --platform tiktok --require-reason "downloaded candidate" --min-score 75 --max-trend-rank 25 --min-downloaded-videos 2 --min-extracted-audios 2 --min-representative-engagement-metrics 2 --min-representative-views 1000000 --min-representative-likes 100000 --recommended-action shortlist_after_rights_review --exclude-risk "Rights still need" --max-risk-count 1 --top 3
```
