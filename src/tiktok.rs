...
pub fn judge_sound_library(manifest_path: &Path) -> Result<Vec<JudgedSound>> {
    ...
    annotate_song_id_best_representative_comment_rates(&mut sounds);
...
fn annotate_song_id_best_representative_like_counts(sounds: &mut [JudgedSound]) {
    let mut song_best_likes = BTreeMap::<String, u64>::new();

    for sound in sounds.iter() {
        if let (Some(song_id), Some(like_count)) = (
            sound
                .song_id
                .as_deref()
                .map(str::trim)
            ...
            song_best_likes
                .entry(song_id.to_string())
                .and_modify(|best_like_count| *best_like_count = (*best_like_count).max(like_count))
                .or_insert(like_count);
        }
    }

    for sound in sounds.iter_mut() {
        sound.song_id_best_representative_like_count = sound
            .song_id
            .as_deref()
            .map(str::trim)
            .filter(|song_id| !song_id.is_empty())
...
fn annotate_song_id_best_representative_comment_rates(sounds: &mut [JudgedSound]) {
    let mut song_best_comment_rates = BTreeMap::<String, u64>::new();

    for sound in sounds.iter() {
        if let (Some(song_id), Some(comment_rate_per_1000_views)) = (
            sound
                .song_id
                .as_deref()
                .map(str::trim)
            ...
            song_best_comment_rates
                .entry(song_id.to_string())
                .and_modify(|best_comment_rate_per_1000_views| {
                    *best_comment_rate_per_1000_views =
                        (*best_comment_rate_per_1000_views).max(comment_rate_per_1000_views)
                })
                .or_insert(comment_rate_per_1000_views);
        }
    }

    for sound in sounds.iter_mut() {
        sound.song_id_best_representative_comment_rate_per_1000_views = sound
            .song_id
            .as_deref()
            .map(str::trim)
            .filter(|song_id| !song_id.is_empty())
            .and_then(|song_id| song_best_comment_rates.get(song_id).copied());
    }
}
...
fn annotate_song_id_best_representative_comment_counts(sounds: &mut [JudgedSound]) {
    let mut song_best_comments = BTreeMap::<String, u64>::new();

    for sound in sounds.iter() {
        if let (Some(song_id), Some(comment_count)) = (
            sound
                .song_id
                .as_deref()
                .map(str::trim)
            ...
            song_best_comments
                .entry(song_id.to_string())
                .and_modify(|best_comment_count| {
                    *best_comment_count = (*best_comment_count).max(comment_count)
                })
                .or_insert(comment_count);
        }
    }

    for sound in sounds.iter_mut() {
        sound.song_id_best_representative_comment_count = sound
            .song_id
            .as_deref()
            .map(str::trim)
            .filter(|song_id| !song_id.is_empty())
            .and_then(|song_id| song_best_comments.get(song_id).copied());
    }
}
...
mod tests {
    ...
    #[test]
    fn annotate_song_id_best_representative_like_counts_uses_highest_likes_per_song() {
        let mut biggest = judged_sound("sound_biggest", 95, Some(4));
        biggest.song_id = Some("shared_song".to_string());
        biggest.representative_like_count = Some(7_427_697);

        let mut smaller = judged_sound("sound_smaller", 90, Some(19));
        smaller.song_id = Some("shared_song".to_string());
        smaller.representative_like_count = Some(175_000);

        let mut local = judged_sound("sound_local", 80, Some(33));
        local.song_id = Some("local_song".to_string());
        local.representative_like_count = Some(225_000);

        let mut missing_likes = judged_sound("sound_missing_likes", 75, None);
        missing_likes.song_id = Some("missing_likes_song".to_string());
        missing_likes.representative_like_count = None;

        let mut missing_song = judged_sound("sound_missing_song", 70, Some(12));
        missing_song.song_id = None;
        missing_song.representative_like_count = Some(8_500_000);

        let mut sounds = vec![biggest, smaller, local, missing_likes, missing_song];
        annotate_song_id_best_representative_like_counts(&mut sounds);

        let best_likes = sounds
            .iter()
            .map(|sound| {
                (
                    sound.sound_id.as_str(),
        ...
        assert_eq!(best_likes.get("sound_biggest"), Some(&Some(7_427_697)));
        assert_eq!(best_likes.get("sound_smaller"), Some(&Some(7_427_697)));
    ...
    #[test]
    fn annotate_song_id_best_representative_comment_rates_uses_highest_density_per_song() {
        let mut biggest = judged_sound("sound_biggest", 95, Some(4));
        biggest.song_id = Some("shared_song".to_string());
        biggest.representative_comment_rate_per_1000_views = Some(7);

        let mut smaller = judged_sound("sound_smaller", 90, Some(19));
        smaller.song_id = Some("shared_song".to_string());
        smaller.representative_comment_rate_per_1000_views = Some(2);

        let mut local = judged_sound("sound_local", 80, Some(33));
        local.song_id = Some("local_song".to_string());
        local.representative_comment_rate_per_1000_views = Some(4);

        let mut missing_rate = judged_sound("sound_missing_rate", 75, None);
        missing_rate.song_id = Some("missing_rate_song".to_string());
        missing_rate.representative_comment_rate_per_1000_views = None;

        let mut missing_song = judged_sound("sound_missing_song", 70, Some(12));
        missing_song.song_id = None;
        missing_song.representative_comment_rate_per_1000_views = Some(9);

        let mut sounds = vec![biggest, smaller, local, missing_rate, missing_song];
        annotate_song_id_best_representative_comment_rates(&mut sounds);

        let best_comment_rates = sounds
            .iter()
            .map(|sound| {
                (
                    sound.sound_id.as_str(),
        ...
        assert_eq!(best_comment_rates.get("sound_biggest"), Some(&Some(7)));
        assert_eq!(best_comment_rates.get("sound_smaller"), Some(&Some(7)));
        assert_eq!(best_comment_rates.get("sound_local"), Some(&Some(4)));
        assert_eq!(best_comment_rates.get("sound_missing_rate"), Some(&None));
        assert_eq!(best_comment_rates.get("sound_missing_song"), Some(&None));
    }
    ...
    #[test]
    fn annotate_song_id_best_representative_comment_counts_uses_highest_comments_per_song() {
        let mut biggest = judged_sound("sound_biggest", 95, Some(4));
        biggest.song_id = Some("shared_song".to_string());
        biggest.representative_comment_count = Some(125_000);

        let mut smaller = judged_sound("sound_smaller", 83, Some(12));
        smaller.song_id = Some("shared_song".to_string());
        smaller.representative_comment_count = Some(5_000);

        let mut local = judged_sound("sound_local", 77, Some(24));
        local.song_id = Some("local_song".to_string());
        local.representative_comment_count = Some(8_000);

        let mut missing_comment_count = judged_sound("sound_missing_comment_count", 68, Some(30));
        missing_comment_count.song_id = Some("missing_comment_song".to_string());
        missing_comment_count.representative_comment_count = None;

        let mut missing_song = judged_sound("sound_missing_song", 70, Some(12));
        missing_song.song_id = None;
        missing_song.representative_comment_count = Some(250_000);

        let mut sounds = vec![biggest, smaller, local, missing_comment_count, missing_song];
        annotate_song_id_best_representative_comment_counts(&mut sounds);

        let best_comments = sounds
            .iter()
            .map(|sound| {
                (
                    sound.sound_id.as_str(),
        ...
        assert_eq!(best_comments.get("sound_biggest"), Some(&Some(125_000)));
        assert_eq!(best_comments.get("sound_smaller"), Some(&Some(125_000)));
        assert_eq!(best_comments.get("sound_local"), Some(&Some(8_000)));
        assert_eq!(
            best_comments.get("sound_missing_comment_count"),
            Some(&None)
        );
        assert_eq!(best_comments.get("sound_missing_song"), Some(&None));
    }
