...
- returned sounds now include `song_id_best_representative_view_count`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed view count that song reached anywhere in the saved library
- `--min-song-id-best-representative-views <n>` lets shortlists require song-level scale even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_like_count`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed like scale that song reached anywhere in the saved library
- `--min-song-id-best-representative-likes <n>` lets shortlists require song-level like scale even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_like_rate_per_1000_views`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed like density that song reached anywhere in the saved library
- `--min-song-id-best-representative-like-rate-per-1000-views <n>` lets shortlists require song-level like density even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_comment_rate_per_1000_views`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed discussion density that song reached anywhere in the saved library
- `--min-song-id-best-representative-comment-rate-per-1000-views <n>` lets shortlists require song-level discussion density even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_engagement_count`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed engagement depth that song reached anywhere in the saved library
- `--min-song-id-best-representative-engagements <n>` lets shortlists require song-level engagement depth even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_comment_count`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed discussion scale that song reached anywhere in the saved library
- `--min-song-id-best-representative-comments <n>` lets shortlists require song-level discussion scale even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_share_count`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed raw spread scale that song reached anywhere in the saved library
- `--min-song-id-best-representative-shares <n>` lets shortlists require song-level raw spread scale even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_engagement_rate_per_1000_views`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed engagement density that song reached anywhere in the saved library
- `--min-song-id-best-representative-engagement-rate-per-1000-views <n>` lets shortlists require song-level engagement density even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- returned sounds now include `song_id_best_representative_share_rate_per_1000_views`, computed from the full judged library before filters, so a deduped shortlist row can still show the strongest observed spread density that song reached anywhere in the saved library
- `--min-song-id-best-representative-share-rate-per-1000-views <n>` lets shortlists require song-level spread density even after `--distinct-song-id` collapses duplicate rows, instead of trusting only the kept representative market sample
- `--distinct-song-id` collapses repeated rows with the same non-empty `song_id` after all other shortlist filters and before `--top`, so a viral pass can return one representative row per song instead of several country-specific copies
