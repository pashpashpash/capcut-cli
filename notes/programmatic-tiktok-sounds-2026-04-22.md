...
- judgement summaries now include representative music duration bands, representative music read/reuse/original-sound/commercial-right-type/strong-beat counts, and `music_vid` coverage, so agents can compare operational music metadata coverage across a library before opening individual rows
